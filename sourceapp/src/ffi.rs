...
#[no_mangle]
pub extern "C" fn get_source_data(username_raw: *const c_char, password_raw: *const c_char, download_path: *const c_char) -> *mut c_char {
    ...
    let data_res = gsd(username.to_str().expect("can't get cstr"), password.to_str().expect("can't get cstr"), download_path.to_str().expect("can't get cstr"));
